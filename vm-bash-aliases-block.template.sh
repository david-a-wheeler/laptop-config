...
# so everything below - noclaude(), run_heroku(), LAPTOP_CONFIG_AUTH_SESSION -
...
# HEROKU_API_KEY, in case a run_heroku (below) subshell is still live in
...
# holds the key itself. Meant for a burst of heroku commands: run_heroku
...
run_heroku() {
    ...
    key="$(secrets-client.py get heroku-api-key --context "run_heroku: ${*:-interactive shell}")" || return 1
