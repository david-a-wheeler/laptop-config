...
until revoked. This finds and revokes those automatically (never the one
you just created):

~~~
heroku login
old_ids="\$(heroku authorizations -j | jq -r --arg desc "$description" \\
  '.[] | select(.description | startswith("laptop-config")) |
   select(.description != \$desc) | .id')"
if [ -n "\$old_ids" ]; then
  printf '%s\n' "\$old_ids" | while IFS= read -r id; do
    heroku authorizations:revoke "\$id"
  done
else
  echo "No old laptop-config authorizations to revoke."
fi
heroku logout
~~~
EOF
