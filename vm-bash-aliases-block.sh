...
# so everything below (BULKHEAD_AUTH_SESSION) is only available from
...
if [ -z "$BULKHEAD_AUTH_SESSION" ]; then
    ...
    # declare/assign for.
    export BULKHEAD_AUTH_SESSION="session_$(head -c 16 /dev/urandom | xxd -p)"
fi
